...
\data{2021} %%Colocar APENAS o ano da entrega dentro das chaves {}. Por exemplo, 2019.
...
\natureza{\printaobjeto ~com o objetivo da obtenção do grau de Bacharel em \printaprograma ~do \printainstituicao } %% Não alterar
...
%% Capa do Trabalho. Não está configurada atualmente, manter comentada a linha debaixo
%\printacapa
...
Palavras-chave: As palavras-chave devem figurar logo abaixo do resumo, antecedidas da expressão “Palavras-chave:”, separadas entre si por ponto e finalizadas também por ponto \cite{bib:abnt6028}. %Finalizadas por ponto e aplicaializadas por letra maiuscula.
\end{resumo_cefetmg}
...
Keywords: Palavra 1. Palavra2. Palavra 3. %Finalizadas por ponto e aplicaializadas por letra maiuscula.
 \end{otherlanguage*}
...
%  Deve obedecer a ordem alfabética. A sigla, quando mencionada pela primeira vez no texto, deve ser indicada entre parênteses, precedida do nome completo (ABNT, 2011). Exemplo: Interface Homem Máquina (IHM).
  \end{siglas}
...
\begin{figure}[h]
	...
	\begin{center}
		\caption[Figura]{Lajes de concreto e detalhes da montagem dos ímãs.}\label{fig:lajes_concreto}
...
\begin{equation}
	\label{eq:Aw0}
	a_{AW0} = \frac{Força~máxima~dos~motores}{Massa~do~veículo~descarregado} = \frac{1800N}{AW0/g}
...
\begin{equation}
	\label{eq:Aw1}
	a_{AW1} = \frac{Força~máxima~dos~motores}{Massa~do~veículo~descarregado} = \frac{1800N}{AW1/g}
...
\begin{figure}[h]
	\larguratexto{16cm}  %Mesma largura da ilustração, dada em "[width=12cm]" abaixo
	\begin{center}
		\caption[Figura]{Lombada e capa.}\label{fig:lajes_concreto}
		\includegraphics[width=16cm]{lombada.jpg}
